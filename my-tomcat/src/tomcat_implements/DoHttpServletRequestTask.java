 ...
 * @date 2020/8/6 17:34
 */
public class DoHttpServletRequestTask implements Runnable {
    private final Socket socket;

    public DoHttpServletRequestTask(Socket socket) {
        this.socket = socket;
    }
    ...
    public void run() {
        //责任列表
        //1. 从socket的输入端，按照 HTTP 协议请求的格式，读取完整的数据，并封装成一个 HttpServletRequest 对象
        //2. 准备好一个空的 HttpServletResponse 对象
        //3. 根据URL，找到合适的 Servlet 对象
        //4. 调用 Servlet 对象的 service(req, resp) 方法，并传入 请求对象 和 响应对象
        //5. 把最终的响应对象，按照 HTTP 协议相应的格式，把数据进行组装，并发到 socket 的输出端
        //6. 为了简单，我们实现成 HTTP/1.0，由服务端主动关闭 socket（TCP的主动关闭方会产生很多的TIME_WAIT状态，所以服务端主动关闭不好）
        ...
        try {
            ...
            //1. 优先在 web.xml 中找动态资源处理的 Servlet 对象
            //2. 如果没找到，看是不是一个静态资源（就看URL对应的静态资源文件是否存在在指定的位置）
            //3. 如果不是静态资源，则使用 404 处理
            ...
            //调用 service 方法：生命周期，service 会被调用好多次
            httpServlet.service(req, resp);

            System.out.println(resp);

            //resp 中的内容已经被填充完毕，所以使用 send 方法，发送到 socket 的输出端中
